...
def main() -> None:
    ...
    # The A1/A2 finalizer writes the authoritative two-tier F2 package.  Only
    # fall back to the legacy generator when no F2 status exists at all.
    if not (OUT / "f2_generation_status.json").exists():
    ...
    gates = {
        ...
        "E2_paraphrase_trust": f2.get("trust_gate") == "FAILED_17_OF_30_RATIFIED"
                               and f2.get("resolution") == "AMENDMENT_A2_TWO_TIER"
                               and f2.get("protocol") == "AMENDMENTS_A1_A2",
    ...
    evidence = {
        ...
        "E2_complete_grid": "`outputs/rebuttal/profile_bootstrap_a1_a2_complete/*/integrity.json`; `outputs/jazz/bootstrap_a1_a2_final.json`",
        "E2_paraphrase_trust": "Jihan D3 and Amendment A2: ratified 17/30 failed gate, strict-fidelity confirmatory subgroup, disclosed exploratory full grid",
    ...
    lines += ["", "## Brief-by-brief audit", "",
              ...
              f"| E2 complete grid and prereg analysis | {'PASS' if gates['E2_complete_grid'] else 'BLOCKED'} | `outputs/rebuttal/profile_bootstrap_a1_a2_complete/*/integrity.json`; `outputs/jazz/bootstrap_a1_a2_final.json`; the old recovery-v1 tree is audit-only. |",
              f"| E2 paraphrase meaning preservation | {'PASS' if gates['E2_paraphrase_trust'] else 'BLOCKED'} | The 17/30 gate remains failed and disclosed; Amendment A2 reports a strict-fidelity confirmatory subgroup plus the labeled exploratory full grid. |",
