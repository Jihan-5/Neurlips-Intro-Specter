#!/usr/bin/env python3
"""Second genuine non-synthetic multi-fault dataset -- Food.com recipes,
SHARED core logic across all 4 models and all 7 arms.

Structural analogue of
`scripts/rebuttal_experiment_c_nonsynthetic_multifault_common.py` (built for
TravelPlanner), reusing the exact same calling convention (per-model wrapper
supplies a `model_table`, this module does example construction + priming +
7-arm running + resumable JSONL writing). See
`intro_specter/benchmarks/recipes_real.py` and
`intro_specter/profiles/recipes_native_multi_fault_injection.py` for the full
dataset/injector design and the honest data-quality finding (only 1,228 of
the advertised 1,048,543 rows in `AkashPS11/recipes_data_food.com` are
non-null; 823 of those carry the three always-required numeric fields).

Feasible fault counts (see `recipes_native_multi_fault_injection.py`
docstring for the underlying measurement):
    N=1,2,3 -- ~823/823 qualifying rows support it
    N=4     -- 819/823 rows (>=1 ingredient parsed)
    N=5     -- 808/823 rows (>=2 distinct ingredients parsed)
This file builds examples by scanning the qualifying-row pool (seeded RNG,
with replacement across seeds, same pattern as `travelplanner_real.py`) and
skipping rows that don't support the requested `num_faults`, reporting the
real achieved count if the pool runs out -- same "report real availability,
don't force" discipline as the TravelPlanner script.

Resolution checking (mechanical, not LLM-judged), per faulted slot, scanned
across the FULL trajectory text (final_output + every step's text -- same
"read the whole trajectory" fix already required for TravelPlanner, since
the day-by-day-plan / recipe-writeup style output routinely puts the
substantive content in a trajectory step rather than `final_output` alone):
  time_limit  -- extracted "Total time: N minutes" (regex) <= true minutes.
  calories    -- extracted "Calories per serving: N" (regex) <= true kcal.
  servings    -- extracted "Servings: N" (regex) == true count, OR a
                 "serves N" / "N servings" mention of the true count.
  excl_ingredient_1/2
              -- final text does NOT contain the TRUE (uncorrupted)
                 ingredient name as a case-insensitive substring.

Output: `outputs/rebuttal/experiment_recipes/{model}/n{N}__{arm}.jsonl`,
...
from intro_specter.benchmarks.recipes_real import (
    _load,
    _qualifying_indices,
    native_constraints,
    parse_r_vector,
...
from intro_specter.profiles.recipes_native_multi_fault_injection import (
...
BENCH_SEED = 42
SUPPORTED_NUMS = {1, 2, 3, 4, 5}

_MIN_RE = re.compile(r"total\s*time[^\d]{0,20}(\d+(?:\.\d+)?)\s*min", re.IGNORECASE)
_CAL_RE = re.compile(r"calories\s*(?:per\s*serving)?[^\d]{0,20}(\d+(?:\.\d+)?)", re.IGNORECASE)
_SERV_RE = re.compile(r"servings?[^\d]{0,20}(\d+)", re.IGNORECASE)
_SERVES_RE = re.compile(r"serves?\s+(\d+)", re.IGNORECASE)


def _extract_minutes(text: str) -> float | None:
    m = list(_MIN_RE.finditer(text or ""))
    if m:
        return float(m[-1].group(1))
    return None


def _extract_calories(text: str) -> float | None:
    m = list(_CAL_RE.finditer(text or ""))
    if m:
        return float(m[-1].group(1))
    return None
...
def _extract_servings(text: str) -> int | None:
    m = list(_SERV_RE.finditer(text or ""))
    if m:
        return int(m[-1].group(1))
    m2 = list(_SERVES_RE.finditer(text or ""))
    if m2:
        return int(m2[-1].group(1))
    return None


def _trajectory_full_text(trajectory: Trajectory) -> str:
    """Same 'scan the whole trajectory, not just final_output' fix already
    required for TravelPlanner's non-synthetic multi-fault script -- agents
    routinely put the actual recipe writeup in a trajectory step rather than
    (or in addition to) `final_output`."""
...
def _resolved_time_limit(full_text: str | None, true_minutes: Any) -> bool:
    got = _extract_minutes(full_text or "")
    return got is not None and true_minutes is not None and got <= float(true_minutes)


def _resolved_calories(full_text: str | None, true_cal: Any) -> bool:
    got = _extract_calories(full_text or "")
    return got is not None and true_cal is not None and got <= float(true_cal)


def _resolved_servings(full_text: str | None, true_serv: Any) -> bool:
    got = _extract_servings(full_text or "")
    return got is not None and true_serv is not None and int(got) == int(true_serv)


def _resolved_ingredient(full_text: str | None, true_ingredient: Any) -> bool:
    true_str = str(true_ingredient or "").strip().lower()
    if not true_str:
        return True
    return true_str not in (full_text or "").lower()


_RESOLVERS: dict[str, Callable[[str | None, Any], bool]] = {
    "time_limit": _resolved_time_limit,
    "calories": _resolved_calories,
    "servings": _resolved_servings,
    "excl_ingredient_1": _resolved_ingredient,
    "excl_ingredient_2": _resolved_ingredient,
}
...
def _make_multi_rule(category: str, mfr: RecipeNativeMultiFaultRecord):
    def rule(profile, task, trajectory, final_output):
        text = _trajectory_full_text(trajectory)
        text_l = text.lower()
        out: list[ViolationEvent] = []
        if category and category.lower() not in text_l:
            out.append(ViolationEvent(
                violation_id="v_recipesnative_mf_no_category",
                ...
                explanation="category missing from output",
            ))
        for slot in mfr.faulted_slots:
            ...
            if not resolver(text, true_val):
                out.append(ViolationEvent(
                    violation_id=f"v_recipesnative_mf_{slot}_unresolved",
                    step_id=trajectory.steps[-1].step_id if trajectory.steps else 0,
                    violated_constraint=f"recipe must satisfy TRUE {slot}={true_val!r} "
        ...
        return out
    rule.__name__ = "recipes_nonsynthetic_multifault_rule"
    return rule


def _build_multifault_examples(
    n_examples: int, seed: int, num_faults: int, scan_limit: int | None = None,
) -> list[tuple[BenchmarkExample, RecipeNativeMultiFaultRecord]]:
    ...
    scan_limit = scan_limit or (len(qualifying) * 4)

    out: list[tuple[BenchmarkExample, RecipeNativeMultiFaultRecord]] = []
    import random
    scan_idx = 0
    while len(out) < n_examples and scan_idx < scan_limit:
        rng = random.Random(seed * 1_000_033 + scan_idx)
        row_idx = qualifying[rng.randrange(len(qualifying))]
        ex = ds[row_idx]
        scan_idx += 1
        if max_faultable_slots(ex) < num_faults:
            continue

        task_id = f"recipes_nonsynth_mf{num_faults}_{scan_idx:06d}"
        mfr = build_recipe_native_multi_fault(task_id=task_id, ex=ex, num_faults=num_faults)
        profile = UserProfile(user_id=f"recipes_native_mf_{task_id}", spans=mfr.spans)

        ingredients = parse_r_vector(ex.get("RecipeIngredientParts"))
        category = ex.get("RecipeCategory") or ""

        prompt = (
            "User dietary/recipe constraints (native Food.com fields):\n"
            + "\n".join(f"- {s.text}" for s in mfr.spans)
            + f"\n\nReal ingredient list on file (for reference, {len(ingredients)} items): "
            f"{', '.join(ingredients[:15])}" + (" ..." if len(ingredients) > 15 else "")
        ...
        rule = _make_multi_rule(category=category, mfr=mfr)

        task = {
            "task_id": task_id,
            "task_type": "recipes_nonsynthetic_multifault",
            "condition": f"recipe_native_n{num_faults}_fault",
            "prompt": prompt,
            "split": "all",
        ...
        bench_example = BenchmarkExample(
            task_id=task_id,
            dataset="recipes_nonsynthetic_multifault",
            profile=profile,
            task=task,
...
# ---------------------------------------------------------------------------
# Priming + 7-arm runners (identical calling convention to the TravelPlanner
# non-synthetic multi-fault script)
# ---------------------------------------------------------------------------
...
def _recover_placeholder_final_output(final_traj: Trajectory, primed: Trajectory) -> Trajectory:
    """Same fix already found necessary for TravelPlanner's non-synthetic
    multi-fault script (`intro_specter/repair.py`'s downstream-regeneration
    fallback can return a non-empty placeholder string, e.g. 'Total time: ...
    minutes' with no number, silently overwriting an already-correct primed
    answer). Mechanical heuristic: if the pipeline output has none of the
    three parseable summary figures but the primed trajectory does, prefer
    the primed trajectory's final_output."""
    def _has_any_figure(text: str | None) -> bool:
        text = text or ""
        return (
            _extract_minutes(text) is not None
            or _extract_calories(text) is not None
            or _extract_servings(text) is not None
        )
    if _has_any_figure(final_traj.final_output):
        return final_traj
    if not _has_any_figure(primed.final_output):
        return final_traj
...
def run_cli(*, model_table: dict[str, tuple[str, str]], default_model: str, default_output_dirname: str) -> None:
    ...
    ap.add_argument(
        "--output-dir",
        default=f"outputs/rebuttal/experiment_recipes/{default_output_dirname}",
    )
    ...
    print(f"Building {n_examples} recipes native-multi-fault (N={args.num_faults}) examples "
