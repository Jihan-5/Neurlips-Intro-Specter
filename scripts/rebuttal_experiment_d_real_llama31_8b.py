#!/usr/bin/env python3
"""Experiment D (real), Llama-3.1-8B extension of `rebuttal_experiment_d_real.py`.

Why this file exists (additive-only, never edits the original)
----------------------------------------------------------------
`scripts/rebuttal_experiment_d_real.py` is tracked and UNMODIFIED. This is a
fresh sibling file for the parallel model-specific rebuttal campaign leg
scoped to Llama-3.1-8B (served via OpenRouter -- NOT Together, which is
reserved for the Llama-3.3-70B leg run by a different agent). It duplicates
`rebuttal_experiment_d_real.py`'s N=2/3/4 fault-construction machinery
verbatim (same `twowiki_real` benchmark, same mechanically-checkable
profile-fault registry, same `build_multi_fault` call) and extends the arm
set from {reflexion, intro_specter, intro_specter_fixed} to the full
7-baseline set requested for this campaign leg:

  direct, self_refine, full_regen, react, selfcheckgpt, reflexion, intro_specter

`tot` and `intro_specter_fixed` are intentionally excluded (cost-justified /
future-work-only exclusions per the campaign brief). All 5 new arms call the
existing, unmodified `intro_specter/baselines/*` implementations with the
exact calling convention `intro_specter/runner.py` uses (verified by reading
`runner.py` lines ~241-387 before writing this file) -- no baseline logic is
reimplemented here.

Usage:
    python3 scripts/rebuttal_experiment_d_real_llama31_8b.py --smoke --num-faults 2
    python3 scripts/rebuttal_experiment_d_real_llama31_8b.py --num-faults 3 --seeds 0,1,2
"""
...
def _with_retry(fn: Callable[[], _T], *, attempts: int = 5, base_delay: float = 3.0, label: str = "") -> _T:
    """Same retry-with-backoff pattern as `rebuttal_experiment_a.py` / `_b.py` /
    `_d_real.py`, with a slightly higher attempt count / base delay since
    OpenRouter's Llama-3.1-8B route has been observed to hit transient
    rate-limit walls under sustained parallel load."""
...
MODEL_TABLE = {
    ...
    # reserved for the sibling Llama-3.3-70B leg run by a different agent).
...
# ---------------------------------------------------------------------------
# Build the N-fault-injected example set (unmodified copy)
# ---------------------------------------------------------------------------
...
def _build_multi_fault_examples(
    ...
    if num_faults < 2 or num_faults > MAX_SUPPORTED_NUM_FAULTS:
        raise ValueError(f"num_faults={num_faults} not supported by this script (2,3,4 only; use the _hop sibling for 5)")
...
def _llm_regenerate_fn(provider_name: str, model: str, seed: int, cache: SQLiteCache | None):
    """Unmodified copy of `intro_specter/runner.py::_llm_regenerate_fn` (that
    file is tracked, so this is a fresh copy, not an edit) -- builds a
    regenerate_fn that calls the agent prompt with a varied seed so
