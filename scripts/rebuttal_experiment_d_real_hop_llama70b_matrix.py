#!/usr/bin/env python3
"""Experiment D (real, hop-fault variant), FULL 7-ARM MATRIX, llama-3.3-70b via
Together, N=5 (and N=6, supported but out of this campaign's requested scope).

Why this file exists
-----------------------
ADDITIVE sibling of `scripts/rebuttal_experiment_d_real_hop.py` (mistral-
nemo-12b, OpenRouter, 2 arms: reflexion / intro_specter). Does NOT edit that
file. Reuses the same, unmodified `intro_specter.profiles.hop_fault_injection
.build_hop_multi_fault` mechanism (4 independent hop-context faults from
2WikiMultiHopQA `bridge_comparison` rows + N-4 layered profile-constraint
faults) to reach N=5, against llama-3.3-70b served via Together (NOT
OpenRouter -- see `intro_specter/models/together_provider.py`), with the
same widened 7-arm set as the N=2/3/4 sibling
(`scripts/rebuttal_experiment_d_real_llama70b_matrix.py`): `direct`,
`self_refine`, `full_regen`, `react`, `selfcheckgpt`, `reflexion`,
`intro_specter`. `tot` and `intro_specter_fixed` are skipped per instruction.

Model / provider: `meta-llama/Llama-3.3-70B-Instruct-Turbo` via `together`,
matching the pairing already used for this model elsewhere in the repo
(e.g. `configs/real/real_truthfulqa_real__llama-3.3-70b.yaml`).

Fault construction / resolution-scoring logic is an unmodified copy of
`rebuttal_experiment_d_real_hop.py`'s logic (all fault-injection machinery
is imported, not copied or modified). Baseline-arm defaults are copied
verbatim from the same project convention cited in the N=2/3/4 sibling's
docstring (reflexion max_trials=2, full_regen max_attempts=1, react
max_steps=4, selfcheckgpt n_samples=5/sample_temperature=1.0, self_refine
defaults).
...
Usage:
    python3 scripts/rebuttal_experiment_d_real_hop_llama70b_matrix.py --smoke --num-faults 5
    python3 scripts/rebuttal_experiment_d_real_hop_llama70b_matrix.py --num-faults 5 --seeds 0,1,2 --n-examples 40
"""
...
def main() -> None:
    ...
    if args.smoke:
        print(
            "Smoke mode: stopping here. Rough cost estimate at Together's Llama-3.3-70B-Instruct-Turbo "
            "~$0.88/M blended token rate (list price at time of writing): "
