#!/usr/bin/env python3
"""Experiment D (real, hop-fault variant), 7-arm extension for qwen-2.5-7b, N=5.

Fresh sibling of `scripts/rebuttal_experiment_d_real_hop.py` (tracked,
UNMODIFIED by this file -- additive-only campaign). Reuses that script's
hop-fault construction/scoring machinery verbatim (see its module docstring
for the full 2WikiMultiHopQA bridge_comparison design rationale -- not
repeated here), but:

  (a) targets qwen-2.5-7b (openrouter, qwen/qwen-2.5-7b-instruct) instead of
      mistral-nemo-12b, and
  (b) adds 5 additional arms (direct, self_refine, full_regen, react,
      selfcheckgpt) on top of the original's (reflexion, intro_specter),
      wired identically to how `rebuttal_experiment_d_real_qwen.py` wires
      them for N=2/3/4 (same baseline calls, same token accounting). `tot`
      is excluded per explicit user instruction; `intro_specter_fixed` is
      out of scope.

This script only runs N=5 (4 hop-context faults + 1 profile-constraint
fault) -- N=6 was out of scope for the task brief (N in {2,3,4,5} only).

Usage:
    python3 scripts/rebuttal_experiment_d_real_hop_qwen.py --smoke
    python3 scripts/rebuttal_experiment_d_real_hop_qwen.py --seeds 0,1,2 --n-examples 40
"""
...
_N_PROFILE_FAULTS_BY_N = {5: 1}
SUPPORTED_NUM_FAULTS = 5
...
def _build_hop_examples(
    ...
    if num_faults not in _N_PROFILE_FAULTS_BY_N:
        raise ValueError(f"num_faults={num_faults} not supported by this script (only 5)")
...
def _run_direct_arm(example, primed, *, provider_name, model, seed, cache) -> dict[str, Any]:
...
def _run_self_refine_arm(example, primed, *, provider_name, model, seed, cache) -> dict[str, Any]:
    provider = build_provider(provider_name, cache=cache)
    verifier = HybridVerifier(rules=list(example.rules))
...
def _run_reflexion_arm(example, primed, *, provider_name, model, seed, cache) -> dict[str, Any]:
    provider = build_provider(provider_name, cache=cache)
    verifier = HybridVerifier(rules=list(example.rules))
...
def _run_full_regen_arm(example, primed, *, provider_name, model, seed, cache) -> dict[str, Any]:
    verifier = HybridVerifier(rules=list(example.rules))
...
def _run_react_arm(example, primed, *, provider_name, model, seed, cache) -> dict[str, Any]:
    provider = build_provider(provider_name, cache=cache)
    verifier = HybridVerifier(rules=list(example.rules))
    result = run_react(
        profile=example.profile, task=example.task, trajectory=primed, verifier=verifier,
...
def _run_selfcheckgpt_arm(example, primed, *, provider_name, model, seed, cache) -> dict[str, Any]:
    provider = build_provider(provider_name, cache=cache)
    verifier = HybridVerifier(rules=list(example.rules))
...
def _run_intro_specter_arm(example, primed, *, provider_name, model, seed, cache) -> dict[str, Any]:
    provider = build_provider(provider_name, cache=cache)
    verifier = HybridVerifier(rules=list(example.rules))
...
def main() -> None:
    ...
    ap.add_argument("--num-faults", type=int, default=5, choices=[5])
